...
class Users extends React.Component {
    ...
    changeStatus = (id, status) => {
        ...
        GlobalProvider.confirmBox("Are you Sure? You want to change status of this account.", (isTrue) => {
            if (isTrue) {
                ApisService.changeUserAccountStatus(id, status)
                    .then(response => {
    ...
    deleteAccount = (id, index) => {
        ...
        GlobalProvider.confirmBox("Are you Sure? You want to delete this account.", (isTrue) => {
            if (isTrue) {
                ApisService.deleteUserAccount(id)
                    .then(response => {
                                                        ...
                                                        <tbody>
                                                            ...
                                                            {entries.map((item, index) =>
                                                                <tr key={item.id} id={'RecordID_' + item.id}>
                                                                    ...
                                                                    <td>
                                                                        <a href={"/users-view/" + item.id} className="btn btn-sm m-b-15 ml-2 mr-2 btn-rounded-circle btn-info" title="Detail"><i className="mdi mdi-eye"></i></a>
                                                                        <a href={"/users-edit/" + item.id} className="btn btn-sm m-b-15 ml-2 mr-2 btn-rounded-circle btn-warning" title="Edit"><i className="mdi mdi-square-edit-outline"></i></a>
